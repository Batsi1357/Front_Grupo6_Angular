...
export class UnidadAddEdit {
  ...
  Grabar() {
    ...
    if (!token) {
      this.snack.open('Debes iniciar sesión antes de crear unidades.', 'OK', { duration: 3000 });
    ...
    if (unidad.idUnidad > 0) {
      this.unidadservice.update(unidad).subscribe({
        next: (data: unidad) => {
          this.snack.open(
            'Se actualizo la unidad con el Id ' + data.idUnidad.toString(),
            'OK',
            { duration: 2000 }
          );
        ...
        error: (http_error) => {
          const msg = http_error?.error?.message || 'No se actualizo el registro solicitado.';
      ...
      this.unidadservice.new(unidad).subscribe({
        next: (data: unidad) => {
          this.snack.open(
            'Se agrego la unidad y se asigno el Id ' + data.idUnidad.toString(),
            'OK',
            { duration: 2000 }
          );
          this.router.navigate(['/unidad-list']);
        ...
        error: (http_error) => {
          const msg = http_error?.error?.message || 'No se agrego el registro solicitado.';
          this.snack.open('ERROR: ' + msg, 'OK', { duration: 5000 });
          console.log(http_error);
        },
      });
